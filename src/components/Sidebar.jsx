...
const Sidebar = ({ onLogout, open, onClose }) => {
  ...
  const MANAGEMENT_TABS = ["Users", "Access", "Doctors Location"];
  const REPORT_TABS = ["Reports"];
  const DATABASE_TABS = ["Db Console"];

  const can = (tabName) => access?.tabs?.includes(tabName);
  const hasAny = (list) => list.some((item) => can(item));
